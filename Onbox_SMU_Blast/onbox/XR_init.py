...
def get_vm_out(ip_address, script_path, lead_xr_vm, vm_type, dirname, vm_count):
...
if __name__ == '__main__':
	...
	for vm in calv:
		t = threading.Thread(target=get_vm_out, args=(vm, '/harddisk\:/gather_info_py2.py', lead_xr, 'sysadmin_'+version, dirname, vm_count))
		threads.append(t)
		t.start()
		vm_count += 1
	for vm in xr:
		t = threading.Thread(target=get_vm_out, args=(vm, '/harddisk\:/gather_info_py2.py', lead_xr, 'xr_'+version, dirname, vm_count))
		threads.append(t)
		t.start()
		vm_count += 1
	...
	try:
		...
		print("Information gathered successfully. output in %s" % (dirname+".tar.gz"))
	except Exception as e:
		print(str(e))
