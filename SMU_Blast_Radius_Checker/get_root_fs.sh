...
xr_iso="$initrd_lv_2/iso/ncs5500-xr.iso"
xr_iso_mnt_pt="$work_dir/mnt_xr_iso"
mkdir $xr_iso_mnt_pt
./extract_iso.sh $xr_iso $xr_iso_mnt_pt

rpm="$xr_iso_mnt_pt/rpm"
cp -r $rpm $work_dir
root_fs="$work_dir/root_fs"
...
cd $pd
rpm="$work_dir/rpm"
rpm2="$giso_mount/xr_rpms"
mv $rpm $root_fs
mv $rpm2 $root_fs
rm -rf $giso_mount
echo "Preparing RPM extraction"
#chroot $root_fs rpm -iv --nodeps /rpm/xr/*.rpm
rpm="$root_fs/rpm/xr/*.rpm"
rpm2="$root_fs/xr_rpms/*.rpm"
RPM="$root_fs/RPM"
mkdir $RPM
cd $root_fs
for r in $rpm
